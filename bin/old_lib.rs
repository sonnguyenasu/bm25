...
pub struct BM25Okapi{
    ...
    pub doc_freq: Vec<FxHashMap<String, f64>>,
}
...
impl BM25Okapi{
    pub fn new(corpus: &Vec<String>, k1: f64, b: f64) -> Self{
        ...
        for doc in corpus.iter(){
            ...
            for term in doc.iter(){
                if let Some(_) = added.get(term){
                    continue;
                }else{
                    added.insert(term);
                    *nd.entry(term.clone()).or_insert(0) += 1;
                }
                *doc_freq_map.entry(term.clone()).or_insert(0.0) += 1.0;
            }
            doc_freq.push(doc_freq_map);
        }
        let idf = BM25Okapi::calc_idf(nd, corpus.len() as f64);//HashMap::new();
        for (i,doc) in corpus.iter().enumerate(){
            let scale = doc.len() as f64 / avgdl;
            doc_freq[i].iter_mut().for_each(|(word, freq)| {
                let word_idf = idf.get(word).unwrap();
                *freq = word_idf*(k1+1.0)/(1.0+k1*(1.0-b+b*scale)/(*freq));
            });
            // doc_freq.push(doc_freq_map);
        }
    ...
    pub fn search(&self, query: &str, top_k: usize)-> Vec<(usize,OrderedFloat<f64>)>{
        ...
        for (idx, doc_freq) in self.doc_freq.iter().enumerate(){
